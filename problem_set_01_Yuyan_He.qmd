...
# pre-process the corpus, using word stem
...
## Separately run for bi-gram: set ngram to 2, and the minimum count of bigrams to 10
...
# Convert processed token file to dfm
data_dfm <- t_clean |> dfm()
...
2. I used word stemming to merge similar words, which I think is important in SNS posts, as many of the posts use colloquial language, and the same vocabulary can vary a lot in format. 

3. I separately ran bi-grams to check the most frequent features. Basically, I feel that bi-grams are not really necessary for this data set, therefore, I decided to not keep the bi-grams so as not to complicate the dfm. 
...
2. Reviewing the full distance data frame, I found out that there are many docs share the same Euclidean distance with the reference doc. I guess it could be that after pre-processing, a lot of the short posts were left with only 0-1 feature. 
...
## read top features for positive exemplar
topfeatures(dfm_pos, n = 50)
## read top features for negative exemplar
